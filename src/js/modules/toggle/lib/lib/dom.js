...
export const toggle = (Store) => () => {
  Store.dispatch({ isOpen: !Store.getState().isOpen },
...
export const startToggleLifecycle = (Store) => () => {
  const {
    node, toggles, settings, isOpen, classTarget, animatingClass,
  } = Store.getState();
...
export const toggleAttributes = ({
  toggles, isOpen, classTarget, animatingClass, statusClass,
}) => {
...
const trapTab = (Store, e) => {
  const { focusableChildren } = Store.getState();
...
export const closeProxy = (Store) => () => {
  const {
    settings, isOpen, focusInListener, clickListener,
  } = Store.getState();
...
export const manageFocus = (Store) => () => {
  const {
    isOpen, focusableChildren, settings, lastFocused, keyListener,
  } = Store.getState();
